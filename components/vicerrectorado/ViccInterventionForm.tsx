...
import { ViccIntervention, ViccInterventionType } from '../../types';
import { CloseIcon } from '../icons/Icons';

interface ViccInterventionFormProps {
    isOpen: boolean;
    onClose: () => void;
    onSave: (intervention: { id?: string; date: string; type: ViccInterventionType; summary: string; participants: string[]; agreements: string; }) => void;
    interventionToEdit: ViccIntervention | null;
    studentName: string;
}

const ViccInterventionForm: React.FC<ViccInterventionFormProps> = ({ isOpen, onClose, onSave, interventionToEdit, studentName }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex justify-between items-center mb-4">
                    <div>
                        <h2 className="text-xl font-bold text-gray-800">{interventionToEdit ? 'Editar' : 'Registrar'} Intervención (Vicerrectorado)</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select id="type" name="type" value={formData.type} onChange={handleChange} required className="mt-1 block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500">
                                {Object.values(ViccInterventionType).map(type => <option key={type} value={type}>{type}</option>)}
                            </select>
                        </div>
                    </div>
                    <div>
...
export default ViccInterventionForm;